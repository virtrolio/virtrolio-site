...
export class MsgIoService {
  ...
  constructor(private afs: AngularFirestore, private authService: AuthService, private domSanitizer: DomSanitizer) {
  ...
  async sendMessage(messageTemplate: VirtrolioMessageTemplate, key: string): Promise<void> {
    ...
    if (toExists) {
      ...
      if (keyIsCorrect) {
        ...
        messageTemplate.contents = this.domSanitizer.sanitize(SecurityContext.HTML, messageTemplate.contents);
